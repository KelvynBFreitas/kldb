...
class kloracle:
    ...
    Args:
        ...
        user: Usuario do Oracle. Se nao informado, busca no ambiente.
        password: Senha do Oracle. Se nao informada, busca no ambiente.
        host: Host/IP do Oracle. Se nao informado, busca no ambiente.
        service_name: Service name do Oracle. Se nao informado, busca no ambiente.
    ...
    def __init__(
        ...
        user = user or os.getenv("USERORACLE")
        password = password or os.getenv("SENHAORACLE")
        host = host or os.getenv("IPORACLE")
        service_name = service_name or os.getenv("SCHEMA")
        ...
        if not all([user, password, host, service_name]):
            ...
            raise ValueError(
                "Dados de conexao do Oracle nao encontrados: "
                + ", ".join(missing)
            )
    ...
    def loadOracle(
        ...
        logger.info(
            "loadOracle: '%s' retornou %d linhas.",
            sql_file,
            len(df)
        )
